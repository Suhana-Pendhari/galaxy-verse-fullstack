...
import LaunchMap from '../components/missions/LaunchMap';
      ...
      <div className="relative rounded-3xl overflow-hidden h-[60vh]">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-8 bg-gradient-to-t from-black/80 to-transparent">
          ...
          <div className="flex flex-wrap gap-4 text-lg">
            <span className="px-3 py-1 bg-cosmic-primary/80 rounded-full">
              {mission.organization}
            </span>
            <span className={`px-3 py-1 rounded-full ${
              mission.status === 'Upcoming' ? 'bg-yellow-500/80' :
              mission.status === 'In Progress' ? 'bg-blue-500/80' :
              mission.status === 'Completed' ? 'bg-green-500/80' :
              'bg-red-500/80'
            }`}>
              {mission.status}
            </span>
      ...
      <div className="flex flex-wrap gap-4 justify-between items-center">
        <div className="flex gap-2">
          ...
          >
            ...
            <span>{mission.stats?.likeCount || 0}</span>
          </button>
          ...
          >
            <FaBookmark className={isInWatchlist ? 'fill-current' : ''} />
            <span>{isInWatchlist ? 'In Watchlist' : 'Add to Watchlist'}</span>
          </button>

          <button
            onClick={() => setShowShareDialog(true)}
            className="flex items-center space-x-2 px-4 py-2 rounded-lg border border-cosmic-primary/30 hover:bg-cosmic-primary/20 transition-colors"
          >
            <FaShare />
            <span>Share</span>
          </button>
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="cosmic-card p-6">
            <h2 className="text-2xl font-orbitron font-bold mb-4">Mission Overview</h2>
          ...
          {mission.timeline && mission.timeline.length > 0 && (
            <div className="cosmic-card p-6">
              <h2 className="text-2xl font-orbitron font-bold mb-4">Mission Timeline</h2>
              <div className="space-y-4">
                {mission.timeline.map((event, index) => (
                  <div key={index} className="flex items-start space-x-4">
                    <div className="w-24 text-cosmic-accent font-mono">{event.time}</div>
          ...
          {mission.crew && mission.crew.length > 0 && (
            <div className="cosmic-card p-6">
              <h2 className="text-2xl font-orbitron font-bold mb-4">Crew</h2>
              <div className="grid md:grid-cols-2 gap-4">
                {mission.crew.map((member, index) => (
                  <div key={index} className="flex items-center space-x-3 p-3 bg-cosmic-light/30 rounded-lg">
                    {member.image ? (
                      <img src={member.image} alt={member.name} className="w-12 h-12 rounded-full" />
                    ) : (
                      <div className="w-12 h-12 rounded-full bg-cosmic-primary/30 flex items-center justify-center">
                        <FaUsers />
                      </div>
                    )}
                    <div>
          ...
          {/* Comments Section */}
          <div className="cosmic-card p-6">
            <h2 className="text-2xl font-orbitron font-bold mb-4">Discussion</h2>
        ...
        <div className="space-y-6">
          ...
          {mission.launchSite && (
            <div className="cosmic-card p-6">
              <h2 className="text-xl font-orbitron font-bold mb-4">Launch Site</h2>
              <LaunchMap site={mission.launchSite} />
              <div className="mt-4 space-y-2">
                <p><span className="text-cosmic-accent">Location:</span> {mission.launchSite.name}</p>
                <p><span className="text-cosmic-accent">Country:</span> {mission.launchSite.country}</p>
              </div>
            </div>
          )}
          ...
          {mission.rocket && (
            <div className="cosmic-card p-6">
              <h2 className="text-xl font-orbitron font-bold mb-4">Rocket</h2>
              <div className="space-y-2">
                <p><span className="text-cosmic-accent">Name:</span> {mission.rocket.name}</p>
                <p><span className="text-cosmic-accent">Type:</span> {mission.rocket.type}</p>
                <p><span className="text-cosmic-accent">Stages:</span> {mission.rocket.stages}</p>
                {mission.rocket.height && (
                  <p><span className="text-cosmic-accent">Height:</span> {mission.rocket.height}m</p>
                )}
                {mission.rocket.mass && (
                  <p><span className="text-cosmic-accent">Mass:</span> {mission.rocket.mass.toLocaleString()} kg</p>
                )}
                {mission.rocket.thrust && (
                  <p><span className="text-cosmic-accent">Thrust:</span> {mission.rocket.thrust.toLocaleString()} kN</p>
                )}
              </div>
            </div>
          )}
          ...
          {/* Payload */}
          {mission.payload && mission.payload.length > 0 && (
            <div className="cosmic-card p-6">
              <h2 className="text-xl font-orbitron font-bold mb-4">Payload</h2>
              <div className="space-y-4">
                {mission.payload.map((item, index) => (
                  <div key={index} className="border-b border-cosmic-primary/30 last:border-0 pb-4 last:pb-0">
                    <h3 className="font-semibold text-cosmic-accent">{item.name}</h3>
                    <p className="text-sm text-gray-400">Type: {item.type}</p>
                    {item.mass && <p className="text-sm text-gray-400">Mass: {item.mass} kg</p>}
                    {item.destination && <p className="text-sm text-gray-400">Destination: {item.destination}</p>}
                  </div>
                ))}
              </div>
            </div>
          ...
          {/* Real-time Data (for SpaceX missions) */}
          {realtimeData && (
            <div className="cosmic-card p-6 border-cosmic-accent/50">
              ...
              <div className="space-y-2">
                <p><span className="text-cosmic-accent">Next Update:</span> {realtimeData.window}</p>
                <p><span className="text-cosmic-accent">Webcast:</span> 
